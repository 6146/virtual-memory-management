...
void do_response()
{
	...
	unsigned int pageCat, pageNum, offAddr, i;
	...
	pageNum = ptr_memAccReq->virAddr % 64 / 4;
...
void do_LRU(Ptr_PageTableItem ptr_pageTabIt)
{
	...
	printf("没有空闲物理块，开始进行LRU(页面老化)算法替换页面...\n");
	...
	while (TRUE)
	{
		...
		else if(c=='1'||c=='2')
		{
			...
			do_response();
	                ...
	                for (i = 0; i < PAGE_SUM; i++)
	                {
		            pageTable[i/16][i%16].count >>= 1;
		            pageTable[i/16][i%16].count = pageTable[i/16][i%16].visited
                                                        ? pageTable[i/16][i%16].count | 0x80
                                                        : pageTable[i/16][i%16].count;
                            pageTable[i/16][i%16].visited = FALSE;
	                } 
		}
		else if(c=='x'||c=='X')
			break;
